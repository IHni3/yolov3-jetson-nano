...
def post_process_image(image, layerOutputs, H, W, confidence, threshold, labels, colors):
    ...
    data = []
    for i in nms_idx:
        (x1, y1) = (int(boxes_coord[i, 0]), int(boxes_coord[i, 1]))
        (x2, y2) = (int(boxes_coord[i, 2]), int(boxes_coord[i, 3]))
        width = x2 - x1
        height = y2 - y1
        
        data.append(DetectionData(labels[classes[i]], x1, y1, width, height, scores[i]))
...
def yolo_object_detection(image, net, confidence, threshold, labels, colors):
    ...
    # get used layers
    used_layers = net.getUnconnectedOutLayers()
	
    # get layer names from ln
    ln_out = []
    for x in used_layers:
        ln_out.append(ln[int(x-1)])

    # perform object detection
    logging.verbose("Performing object detection...")
    sw.start()
    layerOutputs = net.forward(ln_out)
    logging.verbose(sw.message("Performing object detection"))

    # postprocess image
    logging.verbose("Postprocessing image...")
    sw.start()
    result = post_process_image(image, layerOutputs, H, W, confidence, threshold, labels, colors)
    logging.verbose(sw.message("Postprocessing image"))
    
    return result

def yolo_object_detection_blob(images, net, confidence, threshold, labels, colors):
    """ Apply YOLO object detection on a image_file.
        images : Input image
        net : YOLO v3 network object
        confidence : Confidence threshold (specified in command line)
    ...
    (numberOfImages, H, W, CH) = images.shape
    logging.verbose("Number of images: {}".format(numberOfImages))

    # preprocess image data with rescaling and resizing to fit YOLO input shape
    # OpenCV assumes BGR images: we have to convert to RGB, with swapRB=True
    logging.verbose("Preprocessing images...")
    sw.start()
    blobs = cv2.dnn.blobFromImages(images, 1 / 255.0, (416, 416), swapRB=True, crop=False)
    logging.verbose(sw.message("Preprocessing images"))
    ...
    sw.start()
    layerOutputs = net.forward(ln_out)
    logging.verbose(sw.message("Object detection"))

    results = []

    logging.verbose("Postprocessing images...")
    sw.start()
    count = 0
    for image in images:
        singleImageLayerOutputs = []

        # concatenate all outputs from all output layers
        for output in layerOutputs:
            singleImageLayerOutputs.append(output[count])
        
        # post process single image
        result = post_process_image(image, tuple(singleImageLayerOutputs), H, W, confidence, threshold, labels, colors)
        results.append(result)
        count += 1

    logging.verbose(sw.message("Postprocessing images"))

    return results

class DetectionData:
    def __init__(self, class_name, x, y, width, height, score):
